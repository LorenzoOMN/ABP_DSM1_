  ...
  try {
    ...
    return res.status(200).json({
      modulos: progresso.map((modulo) => ({
        ...
        desafio_atual:
          Number(modulo.id_modulo) === Number(modulo.modulo_desafio_atual),
        falhas_no_modulo: Number(modulo.falhas_no_modulo || 0),
        tentativas_gastas_total: Number(modulo.tentativas_gastas_total || 0),
...
router.patch(
  ...
  async function (req, res) {
    ...
    try {
      ...
      const progresso = await concluirHistoria(
      ...
      const exameExistente = await findExameExistente(
        req.usuario.id_usuario,
        idModulo,
      );
